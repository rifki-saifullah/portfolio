<script lang="ts">
	...
	onMount(async () => {
		...
		if (result.ok) {
			...
			if (post.contentBlocks && post.contentBlocks.length > 0) {
				blocks = post.contentBlocks.map((b) => ({
				...
				blocks = [{ id: crypto.randomUUID(), type: 'rich-text', value: { id: '', en: '', ja: '' }, lang: 'id' }];
			}
		}
		loading = false;
	});
	...
	async function handleUpdate(event: Event) {
		...
		saving = true;
		const result = await postService.update(postId, {
			title,
		...
		if (result.ok) {
			...
			toastStore.error(result.error?.message || 'Gagal memperbarui postingan.');
		}
	}
...
<svelte:head>
	<title>{m.admin_post_edit_title()}</title>
</svelte:head>

<div
	class="w-full min-h-screen px-6 py-8 flex justify-center selection:bg-primary selection:text-primary-content"
>
	<div class="w-full max-w-3xl mx-auto font-sans">
		<header class="mb-10 text-center sm:text-left">
			<h1 class="text-3xl font-serif font-bold text-primary mb-2">{m.admin_post_edit_title()}</h1>
		</header>

		{#if loading}
			<div class="space-y-6 bg-base-100 p-6 sm:p-8 rounded-2xl border border-base-300 shadow-sm">
				<LoadingSkeleton variant="card" />
				<LoadingSkeleton variant="text-paragraph" />
			</div>
		{:else}
			<form
			...
			>
				...
				<div class="flex flex-col sm:flex-row justify-end gap-3 pt-4 border-t border-base-200">
					...
					<button type="submit" class="btn btn-primary px-8" disabled={saving || uploadingImage}>
